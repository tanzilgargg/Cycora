...
class CycoraGame {
    ...
    return [
      ...
      },
      {
        ...
        builder: () => [
          this.guideTask(
            "Community Loop",
            "Now you run one complete Cycora day from service to community impact.",
            [
              "A meal is served.",
              "A scrap is saved.",
              "A material is discovered.",
              "A textile is made.",
              "A product is designed.",
              "A person receives it.",
            ],
            "You finish when the loop continues beyond the cafe."
          ),
          this.customerPeek([
          ...
          this.loopSummary(),
          this.guideTask("Before Results", "The cafe is no longer just a restaurant.", ["It is a circular design system.", "Sustainability became a chain of decisions.", "What you served, saved, processed, made, and shared all mattered."], "Press Next to see the completed loop."),
        ],
      },
    ];
  }
  ...
  adaptTasksForAge(tasks, level) {
    ...
    if (this.ageGroup === "adults") {
      adapted = adapted.map((task) => this.addAdultSimulationTone(task));
    }
  ...
  addAdultSimulationTone(task) {
    if (task.type === "guide") {
      task.steps = (task.steps || []).concat("Observe the material flow and adjust the process at your own pace.");
    }
    if (task.type === "choice") {
      ...
      .map((level, i) => {
        ...
        return `
          <button type="button" class="level-node ${unlocked ? "" : "locked"} ${done ? "done" : ""}" data-level="${i}" ${unlocked ? "" : "disabled"}>
            ...
            <span class="level-node-act">${this.escapeHTML(level.act || "Cycora day")}</span>
            <strong>${level.mapName}</strong>
            <small>${unlocked ? `${profile.label}: ${level.goal}` : "Locked"}</small>
          </button>
        `;
      })
      .join("");
  ...
  showResults(completed) {
    ...
    const statRows = [
      ...
      ["Scrap memories", String(this.stats.scrapMemories.length)],
      ...
      [profile.scoreLabel, String(this.stats.circularScore)],
  ...
  renderResultsExtras(achievementText) {
    ...
    const materialCards = this.uniqueList(this.stats.materialsRescued)
      ...
      .join("");
    const memoryCards = this.stats.scrapMemories
      .slice(-4)
      .map((memory) => this.renderMemoryOrb(memory, true))
      .join("");
    ...
    extras.innerHTML = `
      ...
      <div class="result-section"><strong>Scrap memories</strong><div class="result-card-grid memory-results">${memoryCards || `<div class="result-item-card small"><span>◌</span><b>Recover scraps to reveal their memories</b></div>`}</div></div>
    `;
  }
