...
class CycoraGame {
  ...
  readUnlocked() {
    ...
    if (Number.isFinite(saved) && saved >= 1) return Math.min(saved, this.levels.length);
    return 1;
  }
  ...
  createLevels() {
    const storyLevels = [
      ...
      {
        id: 3,
        title: "Eco Lab: Fruit Palette Scanner",
        mapName: "Fruit Palette Scanner",
        icon: "Lab",
        act: "Fruits become ingredients",
        area: "Palette scanner",
        goal: "Scan each fruit and choose the dye, fiber, pigment, or texture path that fits its natural palette.",
        seconds: 75,
        sticker: "Material Detective",
        builder: () => [
          this.guideTask(
            "Fruit Palette Scanner",
            "Enter the cozy Eco Lab where fruits become colors, textures, fibers, and fashion ingredients.",
            [
              "Scan one fruit at a time.",
              "Read its identity: pigment, fiber, texture, oil, tannin, cellulose, or dye.",
              "Choose carefully. The right path unlocks stronger clothing pixels later.",
            ],
            "You finish when every fruit has a precise fashion ingredient path."
          ),
          this.labScan("strawberries", ["warm red dye", "soft pink tint", "seed speckle texture"], "Scan reveals warm red palette potential."),
          this.materialMatch("strawberries", "warm red dye", ["dye", "thread", "metal"], "Strawberries release color and speckled texture for fashion pixels.", "dye", "Strawberries are strongest as warm red dye."),
          this.labScan("blueberries", ["cool blue pigment", "purple shadows", "soft stain"], "Scan reveals cool-tone pigment."),
          this.materialMatch("blueberries", "cool pigment", ["pigment", "fiber", "thread"], "Blueberries unlock cool tones for shadows and pattern fill.", "pigment", "Blueberries are best as pigment and color."),
          this.labScan("oranges", ["bright citrus shade", "glossy peel oil", "soft rind texture"], "Scan reveals bright citrus color."),
          this.materialMatch("oranges", "citrus dye", ["dye", "thread", "stitch"], "Oranges produce bright color before they become structure.", "dye", "Oranges unlock citrus dye."),
          this.labScan("bananas", ["plant fiber", "long strands", "soft cream color"], "Scan reveals plant fiber for material base."),
          this.materialMatch("bananas", "plant fiber", ["fiber", "pigment", "metal"], "Banana strands can become woven material.", "fiber", "Bananas unlock plant fiber for cloth."),
          this.labScan("avocados", ["soft green tone", "pink pit dye", "tannin binder"], "Scan reveals dye and binder potential."),
          this.materialMatch("avocados", "binder dye", ["dye", "weave", "metal"], "Avocado dye and tannin help color hold in the cloth.", "dye", "Avocados unlock binder dye."),
          this.guideTask("Before Results", "The fruits have identities now.", ["Strawberries create warm reds.", "Blueberries unlock cool tones.", "Oranges produce bright citrus shades.", "Bananas become plant fiber."], "Press Next to enter Fruit Fashion Pixel Craft."),
        ],
      },
      {
        id: 4,
        title: "Eco Lab: Fruit Fashion Pixel Craft",
        mapName: "Fruit Fashion Craft",
        icon: "Fabric",
        act: "Pixel outfits come alive",
        area: "Pixel craft line",
        goal: "Sort and type process words to turn fruits into natural dyes, crafting materials, and clothing pixel art.",
        seconds: 90,
        sticker: "Fabric Maker",
        builder: () => [
          this.guideTask(
            "Fruit Fashion Pixel Craft",
            "This is the cozy puzzle loop: fruit comes in, process words guide the craft, and clothing pixel art fills section by section.",
            [
              "Receive fruits and production-process words.",
              "Sort or type the correct process order: wash, extract, blend, weave, dye, craft.",
              "Every completed chain unlocks textures, patterns, and colors from the fruit palette.",
            ],
            "You finish when fruits become finished fashion designs."
          ),
          this.ecoIntro("strawberries", "red hoodie pixel art", ["receive", "sort", "wash", "extract", "blend", "weave", "dye", "craft"], "Strawberries -> warm red dye -> cozy hoodie pixels."),
          this.ecoChoice("sort", ["sort", "spin", "pack"], "Sort strawberries by color and texture before washing.", "strawberries", "red hoodie pixel art"),
          this.ecoChoice("wash", ["wash", "dye", "press"], "Wash fruit before extracting dye.", "strawberries", "red hoodie pixel art"),
          this.ecoChoice("extract", ["extract", "weave", "stitch"], "Extract warm red color from the cleaned fruit.", "strawberries", "red hoodie pixel art"),
          this.ecoChoice("blend", ["blend", "trash", "metal"], "Blend color and texture into a smooth crafting material.", "strawberries", "red hoodie pixel art"),
          this.ecoChoice("weave", ["weave", "boil", "skip"], "Weave the material into soft hoodie texture.", "strawberries", "red hoodie pixel art"),
          this.ecoChoice("dye", ["dye", "spin", "pack"], "Dye the hoodie pixels with warm red tones.", "strawberries", "red hoodie pixel art"),
          this.ecoChoice("craft", ["craft", "trash", "hide"], "Craft the final hood, sleeves, and pixel details.", "strawberries", "red hoodie pixel art"),
          this.ecoIntro("blueberries", "cool sneaker pixel art", ["receive", "sort", "wash", "extract", "blend", "weave", "dye", "craft"], "Blueberries -> cool pigment -> sneaker pixels."),
          this.ecoChoice("sort", ["sort", "press", "pack"], "Sort blue fruit for the coolest tones.", "blueberries", "cool sneaker pixel art"),
          this.ecoChoice("wash", ["wash", "weave", "skip"], "Wash before blending pigment.", "blueberries", "cool sneaker pixel art"),
          this.ecoChoice("extract", ["extract", "stitch", "metal"], "Extract blue-purple pigment.", "blueberries", "cool sneaker pixel art"),
          this.ecoChoice("blend", ["blend", "trash", "thread"], "Blend pigment into a smooth print material.", "blueberries", "cool sneaker pixel art"),
          this.ecoChoice("dye", ["dye", "spin", "pack"], "Dye sneaker panels with cool tones.", "blueberries", "cool sneaker pixel art"),
          this.ecoChoice("craft", ["craft", "burn", "hide"], "Craft soles, laces, and highlight pixels.", "blueberries", "cool sneaker pixel art"),
          this.ecoIntro("oranges", "citrus jacket pixel art", ["receive", "sort", "wash", "extract", "blend", "weave", "dye", "craft"], "Oranges -> bright citrus shades -> jacket pixels."),
          this.ecoChoice("sort", ["sort", "trash", "spin"], "Sort oranges for bright peel color.", "oranges", "citrus jacket pixel art"),
          this.ecoChoice("wash", ["wash", "print", "skip"], "Wash before extracting citrus shade.", "oranges", "citrus jacket pixel art"),
          this.ecoChoice("extract", ["extract", "weave", "hide"], "Extract bright citrus color.", "oranges", "citrus jacket pixel art"),
          this.ecoChoice("weave", ["weave", "trash", "boil"], "Weave a jacket base texture.", "oranges", "citrus jacket pixel art"),
          this.ecoChoice("dye", ["dye", "spin", "pack"], "Dye jacket sections with orange highlights.", "oranges", "citrus jacket pixel art"),
          this.ecoChoice("craft", ["craft", "skip", "wet"], "Craft collar, cuffs, and pixel seams.", "oranges", "citrus jacket pixel art"),
          this.ecoIntro("bananas", "futuristic fashion pixel art", ["receive", "sort", "wash", "extract", "blend", "weave", "dye", "craft"], "Bananas -> plant fiber -> futuristic fashion pixels."),
          this.ecoChoice("sort", ["sort", "dye", "pack"], "Sort banana fiber pieces before extraction.", "bananas", "futuristic fashion pixel art"),
          this.ecoChoice("wash", ["wash", "weave", "skip"], "Wash the fiber source.", "bananas", "futuristic fashion pixel art"),
          this.ecoChoice("extract", ["extract", "dye", "trash"], "Extract strong plant strands.", "bananas", "futuristic fashion pixel art"),
          this.ecoChoice("blend", ["blend", "burn", "hide"], "Blend cream fiber with a flexible binder.", "bananas", "futuristic fashion pixel art"),
          this.ecoChoice("weave", ["weave", "grind", "skip"], "Weave the futuristic outfit base.", "bananas", "futuristic fashion pixel art"),
          this.ecoChoice("dye", ["dye", "pack", "metal"], "Dye accents with soft banana gold.", "bananas", "futuristic fashion pixel art"),
          this.ecoChoice("craft", ["craft", "trash", "plain"], "Craft the final futuristic silhouette.", "bananas", "futuristic fashion pixel art"),
          this.guideTask("Before Results", "The fruit fashion board is glowing.", ["Fruits became natural dyes and crafting materials.", "Correct process chains filled clothing pixel art.", "New collections unlock through calm, step-by-step crafting."], "Press Next to design the collection."),
        ],
      },
      {
        id: 5,
        title: "Act III: Collection Studio",
        mapName: "Collection Studio",
        icon: "Studio",
        act: "Design with purpose",
        area: "Design table",
        goal: "Match each fruit-made material to a clothing pixel art design and unlock new collections.",
        seconds: 65,
        sticker: "Studio Designer",
        builder: () => [
          this.guideTask(
            "Collection Studio",
            "The fruit-made materials are ready, but the collection is not complete.",
            [
              "Warm reds can become cozy hoodies and dresses.",
              "Cool tones can become sneakers and jacket details.",
              "Bright citrus shades make bold fashion accents.",
              "Every finished piece unlocks a new collection tile.",
            ],
            "You finish when the clothing design matches the fruit palette."
          ),
          this.studioPlan("Collection rack: strawberry red, blueberry cool, citrus bright, banana fiber, avocado soft green, and mango gold palettes."),
          this.productTask("strawberry red material", "hoodie", ["hoodie", "banner", "rope"], "Warm red fruit color fits a cozy hoodie.", "hoodie -> Berry Cool Collection"),
          this.layoutTask("nest", ["nest", "scatter", "oversize"], "Nest pattern pieces close together to reduce offcuts."),
          this.assemblyTask("stitch", ["stitch", "glue", "skip"], "Stitch the hoodie seams so the pixel shape reads clearly."),
          this.finishTask("label", ["label", "plain", "wet"], "Add a fruit palette label to the finished outfit."),
          this.productTask("blueberry cool material", "sneaker", ["sneaker", "blanket", "rope"], "Cool pigment belongs on sneaker panels and shadows.", "sneaker -> Fruitcore Sneaker Edit"),
          this.layoutTask("panel", ["panel", "scatter", "tiny"], "Place sneaker panels so the cool tones show."),
          this.assemblyTask("stitch", ["stitch", "skip", "glue"], "Stitch laces and soles into the sneaker design."),
          this.productTask("citrus bright material", "jacket", ["jacket", "placemat", "banner"], "Bright citrus shades make strong jacket accents.", "jacket -> Citrus Streetwear Drop"),
          this.layoutTask("panel", ["panel", "scatter", "tiny"], "Cut jacket panels from the bright palette."),
          this.finishTask("patch", ["patch", "soak", "hide"], "Add the citrus patch detail."),
          this.productTask("mango gold material", "dress", ["dress", "tote", "rope"], "Warm mango gold works as a flowing dress palette.", "dress -> Orchard Runway Set"),
          this.assemblyTask("hem", ["hem", "tear", "rush"], "Hem the dress edge so the pixels stay crisp."),
          this.productTask("banana fiber material", "uniform", ["uniform", "scarf", "blanket"], "Plant fiber can hold a futuristic fashion silhouette.", "uniform -> Future Garden Collection"),
          this.finishTask("label", ["label", "wet", "hide"], "Label the futuristic piece with its fruit source."),
          this.guideTask("Before Results", "The collection is finished.", ["The design is no longer blank pixel art.", "It is textured, patterned, and colored by fruit palettes.", "Eco Maker is ready to unlock the collection."], "Press Next for the handoff."),
        ],
      },
      {
        id: 6,
        ...
        builder: () => [
          ...
          this.guideTask(
            ...
            "Move to the Collection Studio and choose a form that fits the palette."
          ),
    ...
    const buildLoopWith = (extraTasks = []) => {
      const tasks = loopLevel.builder();
      const finalGuide = tasks.pop();
      return tasks.concat(extraTasks, finalGuide);
    };
    return [
      {
        ...loopLevel,
        ...
        goal: "Complete the first loop from meal service to community handoff.",
        seconds: 180,
        sticker: "Loop Maker",
      },
      {
        ...loopLevel,
        id: 2,
        title: "Cycora Loop II",
        mapName: "Loop II",
        icon: "Lab",
        act: "Material memory",
        area: "Advanced material flow",
        goal: "Handle more scraps and protect material value with tougher choices.",
        seconds: 210,
        sticker: "Material Loop Maker",
        builder: () =>
          buildLoopWith([
            this.guideTask(
              "Second Loop",
              "This time, do not stop with citrus. A harder Cycora shift asks you to compare materials instead of following only one obvious path.",
              ["Banana peel, avocado pit, coffee grounds, and citrus peel each carry different properties.", "The more materials you compare, the less you can rely on speed alone."],
              "Read the evidence before you choose the process."
            ),
            this.labScan("banana peels", ["cellulose strands", "flexible peel", "fiber memory"], "The scanner finds plant fiber hiding inside the peel."),
            this.withTypingChallenge(
              this.materialMatch("banana peels", "plant thread", ["thread", "dye", "pigment", "trash"], "Banana peel has strand strength, so thread is the careful path.", "thread", "Banana peel works as fiber when the strands are preserved."),
              ["preserve cellulose strands", "prepare a color bath", "stabilize the pigment matrix", "send to discard route"],
              2
            ),
            this.labScan("coffee grounds", ["brown pigment", "print grit", "damp batch"], "Coffee remembers the morning rush and can become a printed surface."),
            this.withTypingChallenge(
              this.materialMatch("coffee grounds", "brown pigment", ["pigment", "thread", "fiber", "dye"], "Grounds are too crumbly for thread but strong as pigment.", "pigment", "Coffee grounds belong in pigment, not yarn."),
              ["stabilize the pigment matrix", "preserve filament route", "recover cellulose fiber", "prepare a color bath"],
              2
            ),
            ...
            this.withTypingChallenge(
              this.ecoChoice("peel", ["peel", "dye", "print", "pack"], "Separate soft peel from the fiber before spinning.", "banana peels", "plant fiber cloth"),
              ["separate peel from pith", "stain with natural color", "screen print the batch", "archive the finished batch"],
              2
            ),
            this.withTypingChallenge(
              this.settingTask("slow", ["slow", "fast", "hot", "high"], "Low spin speed protects the strand.", "banana peels", "plant fiber cloth"),
              ["calibrate a slow spin", "rush the spin speed", "risk thermal shock", "raise the heat too high"],
              2
            ),
            this.withTypingChallenge(
              this.ecoChoice("weave", ["weave", "grind", "dye", "trash"], "Interlace the spun fiber into cloth.", "banana peels", "plant fiber cloth"),
              ["interlace on the loom", "pulverize the batch", "stain with natural color", "send to discard route"],
              2
            ),
            this.withTypingChallenge(
              this.productTask("plant fiber cloth", "tote", ["tote", "scarf", "banner", "blanket"], "Strong plant fiber cloth can carry weight.", "tote -> student grocery kit"),
              ["build a structured carryall", "cut a thermal wrap", "make a signal banner", "quilt an insulation throw"],
              2
            ),
            this.withTypingChallenge(
              this.typingSentenceTask("Type the process sentence to lock in the harder loop.", "I preserve the fiber before I weave the cloth"),
              "I preserve the fiber before I weave the cloth",
              2
            ),
          ]),
      },
      {
        ...loopLevel,
        id: 3,
        title: "Cycora Loop III",
        mapName: "Loop III",
        icon: "Expert",
        act: "Systems challenge",
        area: "Expert circular system",
        goal: "Balance lab evidence, process settings, product fit, and community routing.",
        seconds: 240,
        sticker: "Circular Systems Maker",
        builder: () =>
          buildLoopWith([
            this.guideTask(
              "Third Loop",
              "This final loop is a systems challenge. You are no longer proving that one scrap has value; you are deciding how a whole circular day should behave.",
              ["A wrong material path lowers quality.", "A careless product choice wastes the textile.", "A weak delivery route breaks the community purpose."],
              "Treat every decision as part of the same chain."
            ),
            this.labScan("avocado pits", ["pink dye", "tannin", "slow simmer"], "The scanner finds color, but only if heat is controlled."),
            this.withTypingChallenge(
              this.materialMatch("avocado pits", "pink dye", ["dye", "weave", "thread", "pigment"], "The pit carries tannin and dye potential, not weave strength.", "dye", "Avocado pits need a slow dye process."),
              ["protect the tannin color bath", "interlace on the loom", "preserve filament route", "stabilize the pigment matrix"],
              3
            ),
            ...
            this.withTypingChallenge(
              this.settingTask("low", ["low", "medium", "high", "hot"], "Low heat keeps the pink dye from dulling.", "avocado pits", "soft pink dyed cloth"),
              ["calibrate low heat carefully", "hold controlled heat", "cross the overheat threshold", "risk thermal shock"],
              3
            ),
            this.withTypingChallenge(
              this.ecoChoice("dry", ["dry", "grind", "spin", "pack"], "Dry the dyed cloth before design work.", "avocado pits", "soft pink dyed cloth"),
              ["desiccate the dyed batch", "pulverize the batch", "extrude a filament route", "archive the finished batch"],
              3
            ),
            this.withTypingChallenge(
              this.productTask("soft pink dyed cloth", "scarf", ["scarf", "tote", "banner", "rope"], "Soft dyed cloth belongs close to skin.", "scarf -> senior winter shelf"),
              ["make a thermal wrap scarf", "build a structured carryall", "make a signal banner", "twist into cordage"],
              3
            ),
            this.withTypingChallenge(
              this.assemblyTask("hem", ["hem", "glue", "skip", "tear"], "Finish the scarf edge so it lasts."),
              ["finish the edge binding", "apply an adhesive bond", "bypass the finish step", "stress the cloth until it tears"],
              3
            ),
            this.withTypingChallenge(
              this.deliveryTask("scarf", "winter", 5, "Send the scarf to the winter support shelf."),
              ["route it to shelter support", "send it to the market stall", "place it in winter relief", "return it to the community cafe"],
              3
            ),
            this.withTypingChallenge(
              this.decisionTask("route", "shelter", ["shelter", "market", "restaurant", "winter"], "A warm textile is needed tonight. Which route is most urgent?", "Fairness protected.", null),
              ["prioritize the shelter route", "rotate through the market", "return it to the cafe", "hold it for the winter queue"],
              3
            ),
            this.withTypingChallenge(
              this.typingSentenceTask("Type the systems paragraph to finish the expert loop.", "I test the dye bath, protect the material, and route the scarf where it is needed most"),
              "I test the dye bath, protect the material, and route the scarf where it is needed most",
              3
            ),
          ]),
      },
  ...
  typingSentenceTask(hint, sentence) {
    return { type: "word", mode: "typingSkill", word: sentence, hint, label: "Typing skill check", feedback: "Typing skill up." };
  }
  ...
  withTypingChallenge(task, typeChoices, difficulty = 2) {
    if (!task) return task;
    task.typingDifficulty = difficulty;
    task.customTypingChallenge = true;
    if (task.type === "choice" && typeChoices) {
      task.typeChoices = this.uniqueTypedChoices(typeChoices, task.choices || []);
      const answerIndex = (task.choices || []).indexOf(task.answer);
      if (answerIndex >= 0) task.typeAnswer = task.typeChoices[answerIndex];
    }
    if (task.type === "word" && typeChoices) {
      task.typeWord = Array.isArray(typeChoices) ? typeChoices[0] : typeChoices;
    }
    return task;
  }
  ...
  adaptTasksForAge(tasks, level) {
    ...
    adapted = adapted.concat(this.ageChallengeTasks(level));
    return adapted
      .map((task) => this.applyVocabularyForAge(task))
      .map((task, index) => this.applyLateLoopTypingDifficulty(task, level, index));
  }

  applyLateLoopTypingDifficulty(task, level, index) {
    if (!level || level.id < 2 || !task || (task.type !== "word" && task.type !== "choice")) return task;
    const difficulty = level.id >= 3 ? 3 : 2;
    task.typingDifficulty = Math.max(task.typingDifficulty || 1, difficulty);
    if (task.customTypingChallenge) return task;

    if (task.type === "word") {
      task.typeWord = this.lateLoopTypingWord(task.typeWord || task.word, level.id, index);
    }

    if (task.type === "choice" && task.choices) {
      task.typeChoices = this.uniqueTypedChoices(
        (task.typeChoices || task.choices).map((choice, choiceIndex) =>
          this.lateLoopTypingWord(choice, level.id, index + choiceIndex)
        ),
        task.choices
      );
      const answerIndex = task.choices.indexOf(task.answer);
  ...
  lateLoopTypingWord(word, levelId, index) {
    const key = this.cleanTypingText(word).replace(/[^a-z0-9]/g, "");
    const levelTwo = {
      next: "continue",
      salad: "serve the citrus salad",
      toast: "serve the avocado toast",
      dye: "prepare a color bath",
      thread: "preserve the filament route",
      fiber: "recover cellulose fiber",
      pigment: "stabilize pigment matrix",
      wash: "sanitize",
      extract: "extract the color safely",
      weave: "interlace on the loom",
      dry: "cure the material batch",
      stitch: "finish the seam",
      shelter: "route to shelter support",
      market: "send to market stall",
      winter: "place in winter drive",
      restaurant: "return to the cafe",
    };
    const levelThree = {
      ...levelTwo,
      continue: "continue the systems audit",
      dye: "protect the tannin color bath",
      thread: "preserve filament integrity",
      fiber: "protect cellulose integrity",
      pigment: "stabilize pigment before printing",
      low: "calibrate low heat carefully",
      medium: "hold controlled heat",
      high: "cross the overheat threshold",
      hot: "risk thermal shock",
      hem: "finish the edge binding",
      glue: "apply an adhesive bond",
      skip: "bypass the finish step",
      tear: "stress the cloth until it tears",
      shelter: "prioritize the shelter route",
      market: "rotate through the market",
      winter: "place it in winter relief",
      restaurant: "return it to the community cafe",
    };
    const map = levelId >= 3 ? levelThree : levelTwo;
    if (map[key]) return map[key];
    if (key.length <= 5) return `${key} ${levelId >= 3 ? "sequence check" : "route check"}`;
    if (index % 3 === 0) return `trace the ${key} flow`;
    if (index % 3 === 1) return `check the ${key} matrix`;
    return `confirm the ${key} process`;
  }

  ageChallengeTasks(level) {
    const n = level.id || 1;
    if (this.ageGroup === "kids") {
      return [
        this.decisionTask(
          "material",
          n === 1 ? "dye" : n === 2 ? "thread" : "dye",
          n === 1 ? ["dye", "trash", "thread"] : n === 2 ? ["thread", "trash", "pigment"] : ["dye", "weave", "trash"],
          n === 1 ? "Rio asks: citrus has bright color. What should it become?" : n === 2 ? "Rio asks: banana peel has long strands. What path fits?" : "Rio asks: avocado pit has pink color. What path fits?",
          "Good memory choice."
        ),
      ];
    }
    if (this.ageGroup === "teens") {
      return [
        this.decisionTask(
          "material",
          n === 1 ? "dye" : n === 2 ? "pigment" : "thread",
          n === 1 ? ["dye", "thread", "fiber", "optimize"] : n === 2 ? ["pigment", "fiber", "dye", "verify"] : ["thread", "dye", "route", "batch"],
          n === 1 ? "Citrus shows oil, soft rind, and color release. Which material pathway has the best evidence?" : n === 2 ? "Coffee grounds are crumbly but color-rich. Which pathway protects value?" : "A new fiber batch has strand strength but weak color. Which pathway fits?",
    ...
    if (this.ageGroup === "adults") {
      return [
        this.decisionTask(
          "material",
          n === 1 ? "dye" : n === 2 ? "pigment" : "dye",
          n === 1 ? ["dye", "thread", "calibrate", "document", "landfill"] : n === 2 ? ["pigment", "cellulose", "validate", "interlace", "landfill"] : ["dye", "filament", "stabilize", "allocate", "landfill"],
          n === 1 ? "Audit the citrus stream: high chroma, low tensile strength, clean rind. Which pathway should be approved?" : n === 2 ? "Audit the coffee stream: particulate texture, strong brown color, no strand integrity. Which pathway is defensible?" : "Audit the avocado stream: tannin content, slow color release, heat sensitivity. Which pathway protects material quality?",
          "Process audit passed."
        ),
      ];
    }
    return [];
  ...
  applyVocabularyForAge(task) {
    if (task.type === "word" && task.word) {
      task.typeWord = task.typeWord || this.vocabularyWord(task.word, task.mode);
      if (task.typeWord !== task.word) task.hint = `${task.hint} Type: ${task.typeWord}.`;
    }
    if (task.type === "choice" && task.choices) {
      const hasCustomAnswer = !!task.typeAnswer;
      task.typeAnswer = task.typeAnswer || this.vocabularyWord(task.answer, task.mode);
      task.typeChoices = task.typeChoices || this.uniqueTypedChoices(task.choices.map((choice) => this.vocabularyWord(choice, task.mode)), task.choices);
      const answerIndex = task.choices.indexOf(task.answer);
      if (answerIndex >= 0 && !hasCustomAnswer) task.typeAnswer = task.typeChoices[answerIndex];
    }
    return task;
  }
  ...
  uniqueTypedChoices(words, fallback) {
    ...
    return words.map((word, index) => {
      let typed = this.cleanTypingText(word || fallback[index] || "");
      if (!typed) typed = this.cleanTypingText(fallback[index] || "choice");
      if (!seen.has(typed)) {
        seen.add(typed);
        return typed;
      }
      const original = this.cleanTypingText(fallback[index] || typed);
  ...
  renderScannerCard(task) {
    ...
    return `
      <div class="scanner-card">
        <div class="scanner-head"><span>${this.materialIcon(material)}</span><strong>Fruit palette scanner</strong><em>${material}</em></div>
        ...
        <p>${task.hint || "Look for fruit color, fiber, texture, or dye potential."}</p>
        ${choices}
      </div>`;
  }
  ...
  renderStudioCard(task, active) {
    ...
    return `
      <div class="studio-board">
        <div class="factory-head"><span>${this.productIcon(product)}</span><strong>Collection Studio</strong><em>Pixels ${Math.min(100, this.stats.saved * 12)}%</em></div>
        <div class="studio-table">
          ...
          <div class="product-preview"><span>${this.productIcon(product)}</span><b>${product}</b><small>${task.impact || "collection unlock"}</small></div>
        </div>
        <p>${task.hint || "Choose a clothing design that fits the fruit palette."}</p>
        ${choices}
      </div>`;
  }
  ...
  renderEcoLine(task, activeStep) {
    const fruitCraftLine = ["receive", "sort", "wash", "extract", "blend", "weave", "dye", "craft"];
    const legacyLine = ["collect", "wash", "peel", "extract", "spin", "weave", "dye", "dry", "grind", "mix", "print", "press", "stitch", "pack"];
    const requestedSteps = task.steps || fruitCraftLine;
    const usesLegacyLine = requestedSteps.some((step) => !fruitCraftLine.includes(step)) || (!!activeStep && !fruitCraftLine.includes(activeStep) && !["start", "setting", "match"].includes(activeStep));
    const stationList = usesLegacyLine ? legacyLine : fruitCraftLine;
    const steps = task.steps || stationList;
    const activeKey = stationList.includes(activeStep) ? activeStep : activeStep === "start" ? "receive" : "receive";
    ...
    const product = task.product || "pixel outfit";
    const material = task.material || "fruit palette";
    const maker = activeStep === "craft" ? this.ecoMaker(product) : `<div class="eco-maker"><img class="maker-body-img" src="eco-lab-kitchen-to-closet/assets/Eco-Maker.png" alt="Eco Maker" /><small>Eco Maker</small></div>`;
    const choices = task.choices ? `<div class="recipient-row-mini">${this.typedChoiceTags(task)}</div>` : "";
    const pixelCells = Array.from({ length: 64 }, (_, i) => {
      ...
      const inShape =
        product.includes("sneaker")
          ...
          : product.includes("dress")
            ? ((row >= 1 && row <= 2 && col >= 3 && col <= 4) || (row >= 3 && row <= 6 && col >= 2 && col <= 5) || (row === 7 && col >= 1 && col <= 6))
            : product.includes("jacket") || product.includes("hoodie") || product.includes("uniform")
              ? ((row === 1 && col >= 2 && col <= 5) || (row >= 2 && row <= 5 && col >= 1 && col <= 6) || (row >= 6 && row <= 7 && col >= 2 && col <= 5))
              : row >= 1 && row <= 6 && col >= 1 && col <= 6;
      const fillLimit = Math.ceil(64 * (activeIndex / (stationList.length - 1)));
      return `<span class="${inShape ? "pixel-cell ghost" : "pixel-cell"} ${inShape && i <= fillLimit ? "filled tone-" + (i % 3) : ""}"></span>`;
    }).join("");
    return `
      <div class="eco-lab-board">
        <div class="factory-head"><span>${this.materialIcon(material)}</span><strong>Fruit Fashion Pixel Craft</strong><em>Quality ${this.stats.qualityScore}%</em></div>
        ...
        <div class="lab-floor">
          <div class="processing-tank"><span>${this.machineIcon(activeKey)}</span><b>${activeStep === "setting" ? "settings" : activeStep === "match" ? "ingredient" : activeKey}</b><small>${task.hint || "Move the fruit palette along the craft line."}</small></div>
          ${maker}
          <div class="pixel-fashion-card"><div class="pixel-fashion-grid">${pixelCells}</div><b>${product}</b><small>pixel art progress</small></div>
        </div>
        ${choices}
      </div>`;
  }
  ...
  materialIcon(material) {
    if (/strawberr/.test(material)) return this.assetIcon("eco-lab-kitchen-to-closet/assets/strawberry.png", "strawberries");
    if (/blueberr/.test(material)) return this.assetIcon("eco-lab-kitchen-to-closet/assets/blueberry.png", "blueberries");
    if (/orange|lemon|citrus/.test(material)) return this.assetIcon("eco-lab-kitchen-to-closet/assets/orange.png", "oranges");
    if (/avocado/.test(material)) return this.assetIcon("eco-lab-kitchen-to-closet/assets/avacado.png", "avocados");
    if (/banana/.test(material)) return this.assetIcon("eco-lab-kitchen-to-closet/assets/banana.png", "bananas");
    if (/mango/.test(material)) return this.assetIcon("eco-lab-kitchen-to-closet/assets/mango.png", "mangoes");
  ...
  productIcon(product) {
    return {
      ...
      hoodie: this.assetIcon("eco-lab-kitchen-to-closet/assets/jacket-2.png", "hoodie"),
      sneaker: "👟",
  ...
  materialProperties(material) {
    ...
    if (s.includes("strawberr")) return ["warm red dye", "soft pink tint", "seed speckle texture"];
    if (s.includes("blueberr")) return ["cool blue pigment", "purple shadows", "soft stain"];
  ...
  transformationPath(material, product) {
    if (/strawberr/.test(material)) return "Strawberries -> warm red dye -> hoodie pixel art";
    if (/blueberr/.test(material)) return "Blueberries -> cool pigment -> sneaker pixel art";
    if (/orange/.test(material)) return "Oranges -> citrus dye -> jacket pixel art";
    if (/banana/.test(material)) return "Bananas -> plant fiber -> futuristic fashion pixel art";
  ...
  completeTask() {
    ...
    const profile = this.ageProfile();
    this.stats.typed += 1;
    this.stats.typedChars += this.typingTargetLength(task);
    this.stats.typingDifficultyPoints += task.typingDifficulty || 1;
    ...
    if (task.mode === "eco") {
      const step = task.word || task.answer;
      if (step === "craft" || step === "dry" || step === "weave" || step === "print" || step === "press" || step === "pack") {
        this.stats.lastTextile = task.product;
        if (!this.stats.textilesCreated.includes(task.product)) this.stats.textilesCreated.push(task.product);
      }
    }
    if (task.mode === "eco" && (task.word === "craft" || task.answer === "craft" || task.word === "pack" || task.answer === "pack")) {
      this.stats.saved += 1;
  ...
  finishLevel(reason) {
    ...
    this.stats.typingSkillScore = this.calculateTypingSkillScore();
  ...
  showResults(completed) {
    ...
    const statRows = [
      ["Typing skill", `${this.stats.typingSkillScore}/100`],
  ...
  expectedTypingTasks() {
    return this.tasks.filter((task) => task.type === "word" || task.type === "choice" || task.type === "guide").length || 1;
  }

  typingTargetLength(task) {
    if (!task) return 0;
    if (task.type === "choice") return String(this.typedAnswer(task) || "").length;
    if (task.type === "guide") return String(this.ageProfile().guideWord || "next").length;
    return String(this.typedWord(task) || "").length;
  }

  calculateTypingSkillScore() {
    const completion = Math.min(1, this.stats.typed / this.expectedTypingTasks());
    const totalAttempts = this.stats.typedChars + this.stats.mistakes;
    const accuracy = totalAttempts > 0 ? this.stats.typedChars / totalAttempts : 1;
    const averageDifficulty = this.stats.typed > 0 ? this.stats.typingDifficultyPoints / this.stats.typed : 1;
    const difficultyBonus = Math.min(15, Math.max(0, (averageDifficulty - 1) * 7.5));
    return Math.max(0, Math.min(100, Math.round(completion * 45 + accuracy * 40 + difficultyBonus)));
  }
  ...
  handleTypingKey(e) {
    ...
    if (this.activePrompt && this.activePrompt.type === "choice") return this.choiceTyping.handleKeydown(e);
    return this.typing.handleKeydown(e);
  }
